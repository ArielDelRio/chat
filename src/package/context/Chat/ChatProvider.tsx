...
export const ChatProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const selectContact = async (contactSelected: ContactInput, view?: Views) => {
    ...
    if (view === "on-chat") {
      const conversation = await findConversationByIdentity(
        contactSelected.identity
      );
      if (conversation?.conversation) {
        await selectConversation(conversation.conversation.sid);
      }
    }
  ...
  const startConversation = async (contact: Contact) => {
    ...
    try {
      try {
        ...
        setAlert({
          message: "User is not registered on Conversation Service",
          type: "error",
          context: JSON.stringify(error),
        });
        return;
      ...
      // ! Only work for individual conversations
      const existingConversation = await findConversationByIdentity(
        contact.identity
      );

      if (existingConversation) {
        await openConversation(existingConversation.conversation.sid);
        return;
      }
      ...
      if (contact.type === "identifier") {
        ...
        const TWILIO_PHONE_NUMBER = "+13856007854";
        const BINDING_ADDRESS = contact.identity;
  ...
  const openConversation = async (conversationSid: string) => {
    const client = getClient();

    if (client) {
      await selectConversation(conversationSid);
  ...
  const selectMessage = (
    ...
    if (reason === "download") {
      if (message.attachedMedia?.length) {
        const attachedMedia = message.attachedMedia[0];
        attachedMedia
          ...
          .then(async (url) => {
            ...
            try {
              await downloadMediaFromUrl(url, attachedMedia.filename);
            } catch (error) {
              setAlert({
  ...
  const addClientListeners = (
    ...
    client.on("initialized", async () => {
      ...
      clearAlert();
      events?.onClientInitialized?.(getEventContext());
    });
  ...
  const findConversationByIdentity = async (
    identity: string
    ...
    return conversations.find((conversation) => {
      const { type, partyUsers, partyParticipants } = conversation;
      if (type === "individual") {
        return (
          partyUsers.some((user) => user.identity === identity) ||
          partyParticipants.some((participant) => {
            const participantBindings = participant.bindings as {
              sms: { address: string };
            };
            return participantBindings?.sms?.address?.includes(identity);
          })
        );
      }
    });
  ...
  const selectConversation = async (conversationSid: string) => {
    ...
    const activeConversation = await fetchActiveConversation(conversationSid);
    ...
    while (messagesPaginator.items.length) {
      ...
      for (const message of messages) {
        ...
        if (
          ...
          messageAttribute.tags?.some((tag) =>
  ...
  const getMessageAuthor = (message: Message): Contact => {
    if (!message.author) {
      return new Contact({
        identity: "unknown",
        type: "identifier",
    ...
    if (message.author === chatRef.current.contact.identity) {
      return chatRef.current.contact;
    }

    if (chatRef.current.activeConversation) {
      return getContactFromActiveConversation(message.author);
    }

    const messageAttributes = message.attributes as MessageAttributes;
    const authorObj = JSON.parse(messageAttributes?.data?.author || "null");

    if (authorObj) {
      return new Contact(authorObj);
    }
