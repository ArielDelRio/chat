...
export const ChatForm = ({
  ...
  const { activeConversation, selectedMessage, selectionMode, contact } =
    useChat();
  const { selectMessage, setAlert, clearAlert } = useChatDispatch();
  ...
  const handleSendMessage = async () => {
    ...
    if (cleanMessage && conversation) {
      ...
      const { data, status, ...messageAuthor } = contact;
      ...
      await activeConversation?.conversation
        ...
        .setAttributes({
          data: {
            author: JSON.stringify(messageAuthor),
          },
        })
        .buildAndSend();
