...
export const SelectionChatForm = ({ onClickSelectedMessages }: Props) => {
  ...
  const [openDeleteSelectedMessagesModal, setOpenDeleteSelectedMessagesModal] =
    useState(false);
  const { resetSelectionMode } = useChatDispatch();
      ...
      >
        ...
        <Box display={"flex"} alignItems={"center"} gap={1}>
          <Typography variant="body2">
            <strong>{selectionMode.selectedMessages.length}</strong> selected
          </Typography>
          <Button variant="outlined" size="small" onClick={resetSelectionMode}>
            Cancel
          </Button>
        </Box>
      </Box>
